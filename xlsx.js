...
function get_cell_style(styles, cell, opts) {
	if (typeof style_builder != 'undefined') {
		// If the cell is already an integer index, leave it be.
...
// Define the XmlNode class to use in the StyleBuilder.
var XmlNode = (function() {
	...
	XmlNode.prototype.append = function () {
	...
	XmlNode.prototype.escapeAttributeValue = function(att_value) {
		// TODO: Extend with the four other codes.
...
// Define the StyleBuilder to be able to format XLSX files.
var StyleBuilder = function(options) {
	...
	// Cache style specifications to avoid excessive duplication.
	_hashIndex = {};
	_listIndex = [];
	...
	return {
		initialize: function(options) {
			...
			// Styles at index 0 and 1 must be specified.
		...
		/** 
		 * Creates a style entry and returns an index used in the `cell.s` property.
		 * The formatting of this object follows the emerging Common Spreadsheet Format.
		 */
		...
		/**
		 * Creates style entries and returns an array of indices that used in the `cell.s` property.
		 */
		...
		_duckTypeStyle: function(attributes) {
			...
			if (attributes.font || attributes.numFmt || attributes.border || attributes.fill) {
				return attributes;
			}
		...
		/**
		 * Create an <xf> record for the style, as well as the corresponding
		 * <font>, <fill>, <border>, and <numfmts>
		 *
		 */
		// Right now, this is simple and creates a <font>, <fill>, <border>, and
		// <numfmts> for every <xf>.
		//
		// We could, perhaps, get fancier and avoid duplicating auxiliary entries
		// as Excel presumably intended ... but why bother?
		_addXf: function(attributes) {
			...
			if (attributes.alignment) {
				...
				if (attributes.alignment.vertical) $alignmnet.attr('vertical', attributes.alignment.vertical);
		...
		_addNumFmt: function(numFmt) {
			...
			if (typeof numFmt === 'string') {
				...
				if (numFmtIdx >= 0) {
					// Match found against built-in formats.
					return numFmtIdx;
				}
			}
			if (/^[0-9]+$/.exec(numFmt)) {
		...
		_addFill: function(attributes) {
			...
			if (attributes.fgColor) {
				...
				// Excel does not like when both RGB and Theme + Tint are set,
				// but xlsx.parseFile() sets both.
				...
				if (attributes.fgColor.rgb) {
					if (attributes.fgColor.rgb.length === 6) {
						// Need to prepend with alpha, as Excel expects aRGB.
